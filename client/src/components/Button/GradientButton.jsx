...
const GradientButton = () => {
  ...
  return (
    <button
      ...
      className="px-10 py-3 bg-gradient-to-r from-purple-500 to-purple-900 rounded-full shadow-xl group-hover:shadow-2xl group-hover:shadow-purple-900 shadow-purple-900 uppercase font-serif tracking-widest relative overflow-hidden group text-transparent cursor-pointer z-10 after:absolute after:rounded-full after:bg-sky-700 after:h-[85%] after:w-[95%] after:left-1/2 after:top-1/2 after:-translate-x-1/2 after:-translate-y-1/2 hover:saturate-[1.15] active:saturate-[1.4]"
      onMouseEnter={() => setHover(true)}
      onMouseLeave={() => setHover(false)}
    >
      Button
      <p
        className="absolute z-40 font-semibold bg-gradient-to-r from-white to-white bg-clip-text text-transparent top-1/2 left-1/2 -translate-x-1/2 group-hover:-translate-y-full h-full w-full transition-all duration-300 -translate-y-[30%] tracking-widest"
      >
        WELCOME
      </p>
      <p
        className="absolute z-40 top-1/2 pt-1 left-1/2 bg-gradient-to-r from-white to-white bg-clip-text text-transparent -translate-x-1/2 translate-y-full h-full w-full transition-all duration-300 group-hover:-translate-y-[40%] tracking-widest font-extrabold"
      >
       {isLoggedIn ? "Get To Chat":"Get Started"} 
      </p>
      <svg
        className={`absolute w-full h-full top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-30 ${hover ? 'group-hover:animate-none' : 'group-hover:animate-pulse'} group-hover:-translate-y-[45%] transition-all duration-300`}
        viewBox="0 0 2400 800"
        xmlns="http://www.w3.org/2000/svg"
      >
        <defs>
          <linearGradient id="sssurf-grad" x2="50%" y1="0%" y2="100%">
            <stop offset="0%" stopColor="hsl(210, 65%, 45%)" stopOpacity="1" />
            <stop offset="100%" stopColor="hsl(316, 73%, 52%)" stopOpacity="1" />
          </linearGradient>
        </defs>
      ...
      <svg
        className={`absolute w-full h-full top-1/2 left-1/2 -translate-x-1/2 -translate-y-[30%] group-hover:-translate-y-[33%] group-hover:scale-95 transition-all duration-500 z-40 fill-blue-950 ${hover ? 'group-hover:scale-95' : 'group-hover:scale-100'}`}
